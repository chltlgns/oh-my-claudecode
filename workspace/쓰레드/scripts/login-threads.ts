 ...
 *
 * Usage:
 *   npx tsx scripts/login-threads.ts          # 단독 실행 → stdout JSON
 *   import { loginThreads } from './login-threads.js'  # 모듈로 import
 */
...
const CREDENTIALS_PATH = '/mnt/c/Users/campu/OneDrive/Desktop/새 텍스트 문서 (2).txt';
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    ...
    for (const textPattern of dismissTexts) {
      ...
      if (await btn.count() > 0) {
        ...
        dismissed = true;
        await humanDelay(TIMING.actionDelay);
        break;
      }
    }
    if (!dismissed) break;
