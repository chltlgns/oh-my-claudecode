...
### Step 3.5: 쿠팡 구매 가능성 필터 (HARD GATE)

**이 단계는 반드시 실행한다. 건너뛰지 않는다.**

각 클러스터의 `product_categories`에 들어갈 항목이 **쿠팡에서 검색하면 구매 가능한 물리적 제품**인지 판단한다.

**쿠팡에서 살 수 있는 것** (OK):
- 건강식품, 영양제, 뷰티 제품, 주방용품, 생활가전, 의류, 식품, 문구, 운동용품, 디지털 기기, 유아용품, 인테리어 소품 등 **물리적 제품**

**쿠팡에서 살 수 없는 것** (제외):
- 서비스: 상담, 시술, 임대, 구독 서비스, 코워킹스페이스, 성형외과
- 앱/소프트웨어: 시간관리 앱, 투자 플랫폼, 자동화 서비스
- 교육: 온라인 강의, 코칭, 교육과정
- 금융: 투자 상품, 보험, 재테크
- 부동산: 임대, 매매

**처리 규칙**:
1. `product_categories`의 각 항목을 검사 → 쿠팡 구매 불가 항목은 제거
2. 제거 후 `product_categories`가 빈 배열이 되면 → `purchase_linkage: '하'`로 강제 설정
3. 니즈 자체는 기록하되, 쿠팡에서 해결 불가능한 니즈임을 `why_linkage`에 명시

**예시**:
- "성형수술 결과 불만족" → product_categories: ["성형외과 상담", "모발이식 시술"] → 전부 서비스 → `product_categories: []`, `purchase_linkage: '하'`
- "오후 집중력 저하" → product_categories: ["시간관리 앱", "카페인 보충제"] → "시간관리 앱" 제거, "카페인 보충제" 유지 → `product_categories: ["카페인 보충제"]`
